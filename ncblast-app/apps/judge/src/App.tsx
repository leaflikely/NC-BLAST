...
export function BeyJudgeApp() {
  ...
  const [sheetsStatus, setSheetsStatus] = useState<
    null | "success" | "error" | "queued"
  >(null);
  const [challongeSlug, setChallongeSlug] = useState("");
  const [challongeParticipants, setChallongeParticipants] =
    useState<ChallongeParticipantMap>({});
  ...
  useEffect(() => {
    ...
    const savedMap = sGet(
      KEYS.challongeMap,
      {} as { slug?: string; participants?: ChallongeParticipantMap },
    );
  ...
  const handleDownloadCSV = (
    roundLog: LogEntry[],
    meta: DownloadCsvMeta,
  ): void => {
    let csv =
      "Round,Set,Shuffle,Judge,Tournament,Winner,WinnerCombo,FinishType,Points,Penalty,P1,P1Side,P1Score,P1Combo,P2,P2Side,P2Score,P2Combo,Timestamp\n";
    roundLog.forEach((r) => {
      csv += `${r.round},${r.set},${r.shuffle},"${r.judge || ""}","${meta.config?.tournamentName || ""}","${r.scorer}","${r.winnerCombo}",${r.type},${r.points},${r.penalty ? 1 : 0},"${r.p1Name}",${r.p1Side || ""},"${r.p1Score}","${comboStr(r.p1Combo)}","${r.p2Name}",${r.p2Side || ""},"${r.p2Score}","${comboStr(r.p2Combo)}",${r.time}\n`;
    });
  ...
  const handleSendSheets = async (
    ...
    try {
      ...
      const queueId = enqueue({
        kind: "sheets",
        type: "sheets",
        payload: { rows, battleRows, flagged, comment },
      });
  ...
  useEffect(() => {
    const flush = async (): Promise<void> => {
      ...
      for (const item of queue) {
        if (!item.id) continue;
        const kind =
          item.kind ?? (item.type === "sheets" ? "sheets" : undefined);
        try {
          if (kind === "sheets") {
            const resp = await fetch(SHEETS_URL, {
              method: "POST",
    ...
    >
      ...
      {screen === "players" && (
        <PlayersScreen
          ...
          dark={dark}
          config={config}
          onChallongeImport={(slug, pmap) => {
            setChallongeSlug(slug);
            setChallongeParticipants(pmap);
            sSave(KEYS.challongeMap, { slug, participants: pmap });
          }}
        />
      )}
      {screen === "match" && (
