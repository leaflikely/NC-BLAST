      ...
      {showChangelog && (
        ...
        >
          ...
          >
            ...
            <button
              onClick={() => setShowChangelog(false)}
              style={{ ...S.current.pri, marginTop: 4 }}
            >
              Close
            </button>
          </div>
      ...
      <div style={{ ...S.current.page, height: "100dvh", overflowY: "auto" }}>
        <div style={{ textAlign: "center", marginBottom: 16 }}>
          ...
          >
            ...
            >
              v{CHANGELOG[CHANGELOG.length - 1].v} — Changelog
            </button>
          </div>
        </div>
        <button
          ...
          style={{
            ...
            background: hasLib ? "#15803D08" : "#FFF7ED",
            cursor: "pointer",
            marginBottom: 14,
            textAlign: "left",
          }}
        >
          ...
          <div style={{ flex: 1 }}>
            ...
            >
              {hasLib
                ? `Library: ${parts.blades.length} blades · ${parts.ratchets.length} ratchets · ${parts.bits.length} bits`
                : "No parts loaded — tap to set up"}
            </div>
            <div
              style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 1 }}
            >
              {hasLib ? "Tap to manage" : "Import your Beyblade X parts first"}
            </div>
          </div>
        </button>
        <div style={S.current.card}>
          <h2 style={S.current.label}>Match Type</h2>
          <div style={S.current.row}>
            {(
              [
                [4, "4 Pts."],
                [5, "5 Pts."],
              ...
              <button
                ...
                style={{
                  ...S.current.chip,
                  ...(config.pts === v ? S.current.chipOn : {}),
                }}
                onClick={() => setConfig({ ...config, pts: v })}
              >
                {l}
              </button>
            ))}
          </div>
          <div style={{ ...S.current.row, marginTop: 8 }}>
            <button
              style={{
                ...S.current.chip,
                ...(config.pts > 7 ? S.current.chipOn : {}),
              }}
              onClick={() => {
            ...
            {[1, 3, 5].map((v) => (
              <button
                ...
                style={{
                  ...S.current.chip,
                  ...S.current.chipW,
                  ...(config.bo === v ? S.current.chipBl : {}),
                }}
                onClick={() => setConfig({ ...config, bo: v })}
              >
                {v === 1 ? "Best-of-1" : `Best-of-${v}`}
              </button>
            ))}
          </div>
          ...
          {config.tm && (
            <div style={{ marginTop: 12 }}>
              <input
                ...
                onChange={(e) =>
                  setConfig({ ...config, tournamentName: e.target.value })
                }
              />
              {!config.tournamentName?.trim() && (
                <p
                  style={{ ...S.current.hint, color: "#7C3AED", marginTop: 4 }}
                >
                  Enter tournament name to continue
                </p>
              )}
            </div>
          )}
        ...
        <button
          style={{
            ...S.current.pri,
            opacity:
              hasLib && (!config.tm || config.tournamentName?.trim()) ? 1 : 0.4,
          }}
        ...
        {!hasLib && (
          <p style={S.current.hint}>Set up your parts library first</p>
        )}
      </div>
    </>
  );
